...
int track_getpath(track_node *node, int *path, int start, int end){
    //intitial
    int dist[144],queue[144],parent[144],count=144,i,ind;
    for(i=0;i<144;i++){
        dist[i] = 10000;
        queue[i] = 1;
    }
    ...
    while (count!=0){
        ...
        if(ind == end)
            break;
        queue[ind] = 0;
        track_node *u = &node[ind];
        if(u->type == NODE_SENSOR){
            if(dist[ind]+u->edge[DIR_AHEAD].dist < dist[(int)(u->edge[DIR_AHEAD].dest-node)]){
                dist[(int)(u->edge[DIR_AHEAD].dest-node)] = dist[ind]+u->edge[DIR_AHEAD].dist;
                parent[(int)(u->edge[DIR_AHEAD].dest-node)] = ind;
            }
        }
        ...
        if(u->type == NODE_BRANCH){
            if(dist[ind]+u->edge[DIR_STRAIGHT].dist < dist[(int)(u->edge[DIR_STRAIGHT].dest-node)]){
                dist[(int)(u->edge[DIR_STRAIGHT].dest-node)] = dist[ind]+u->edge[DIR_STRAIGHT].dist;
                parent[(int)(u->edge[DIR_STRAIGHT].dest-node)] = ind;
            }
            if(dist[ind]+u->edge[DIR_CURVED].dist < dist[(int)(u->edge[DIR_CURVED].dest-node)]){
                dist[(int)(u->edge[DIR_CURVED].dest-node)] = dist[ind]+u->edge[DIR_CURVED].dist;
                parent[(int)(u->edge[DIR_CURVED].dest-node)] = ind;
            }
        }
...
short track_getnext(track_node *track, short *switcher,  char c, short n, char *res_c, short *res_n){
    int res = 0;
    short i = sensor_to_track(n,c);
    track_node *next = track[i].edge[DIR_AHEAD].dest;
    res += track[i].edge[DIR_AHEAD].dist;
    while (1) {
        switch (next->type) {
            case NODE_SENSOR: {
                // bwprintf(COM2, "\033[40;19H %s %c%d", next->name, next->name[0],next->num);
                *res_c = next->name[0];
                *res_n = next->num%16+1;
                if(next->dead != 0){
            ...
            case NODE_MERGE: {
                res += next->edge[DIR_AHEAD].dist;
                next = next->edge[DIR_AHEAD].dest;
                break;
            }
            case NODE_BRANCH: {
                if(switcher[switch_to_track(next->num)]==34) {
                    res += next->edge[DIR_CURVED].dist;
                    next = next->edge[DIR_CURVED].dest;
                }
                else if(switcher[switch_to_track(next->num)]==33){
                    res += next->edge[DIR_STRAIGHT].dist;
                    next = next->edge[DIR_STRAIGHT].dest;
                }
                else{
                    bwprintf(COM2,"NEXT SENSOR ERROR NO SWITCH next->name:%s switcher%d\n\r",next->name,switcher[next->num]);
                }
                break;
            }
            default: {
                //bwprintf(COM2,"NO NEXT SENSOR %s\n\r",next->name);
                return res;
            }
        }
        if(res>1000||res<0)
            bwprintf(COM2,"\033[43;15H DISTANCE ERROR %d TYPE:%d\n\r",res,next->type);
    }
}
...
short sensor_to_pre_track(track_node *track, short *switcher, short n, char c){
    char res_c;
    short res_n;
    sensor_reverse(track,c,n,&res_c,&res_n);
    char res_pre_c;
    short res_pre_n;
    track_getnext(track, switcher, res_c, res_n, &res_pre_c, &res_pre_n);
    char pre_c;
    short pre_n;
    sensor_reverse(track,res_pre_c,res_pre_n,&pre_c,&pre_n);
    return sensor_to_track(pre_n,pre_c);
}
...
void track_inita(track_node *track) {
    ...
    track[17].edge[DIR_AHEAD].dist = 215;
    track[18].name = "B3";
    ...
    track[18].edge[DIR_AHEAD].dist = 203;
    track[19].name = "B4";
    ...
    track[32].edge[DIR_AHEAD].dist = 203;
    track[33].name = "C2";
